...
# this could probably also be done inline.
s = open(
    "/Users/rohanparkes/dev/repos/ABSData/Scripts/Data/ABS_C16_T01_TS_SA_08062021164508583.xls",
    mode="r",
    encoding="utf-8-sig",
).read()
open(
    "/Users/rohanparkes/dev/repos/ABSData/Scripts/Data/ABS_C16_T01_TS_SA_08062021164508583.csv",
    mode="w",
    encoding="utf-8",
).write(s)
...
cnxn = pyodbc.connect(
    "DRIVER={ODBC Driver 18 for SQL Server};Server=tcp:127.0.0.1,1433;Database=ABSPopulationData;Uid=stats;Pwd=MyNewStrongPwd1@;TrustServerCertificate=yes;"
)
cursor = cnxn.cursor()
...
if len(states_db_set) == 0:
    for state in set_states:
        ...
        insert_string = "{}'{}',{}{}".format(
            "INSERT INTO DimState(name, ABSStateId) VALUES (", name, id, ");"
        )
...
if len(ages_new_set) > 0:
    for age in ages_new_set:
        ...
        name = dict_ages[age]
        insert_string = "{}'{}',{}{}".format(
            "INSERT INTO DimAge(name, ABSAgeId) VALUES (", name, id, ");"
        )
        cursor.execute(insert_string)
...
if len(regions_new_set) > 0:
    for region in regions_new_set:
        ...
        insert_string = "{}'{}',{}{}".format(
            "INSERT INTO DimRegion(name, ABSRegionId) VALUES (", name, id, ");"
        )
        cursor.execute(insert_string)
...
if len(sexes_new_set) > 0:
    for sex in sexes_new_set:
        ...
        name = dict_sexes[sex]
        insert_string = "{}'{}',{}{}".format(
            "INSERT INTO DimSex(name, ABSSexId) VALUES (", name, id, ");"
        )
        cursor.execute(insert_string)
